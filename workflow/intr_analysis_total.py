...
def get_intr_sections(interactions) -> pd.DataFrame:

    # assign section number as the interaction index. 
    for i in range(len(interactions)):
        interactions.at[i, 'section'] = i
...
def frequency_dict(min_g_vals_group):
    '''
    ...
    max_block = max(max(block) for block in int_blocks)
    freq = np.zeros(max_block + 1).astype(int)
...
def get_block_g_vals(id, min_g_vals_struct_elem, output_dir):
    ...
    blockwise_g_vals.to_csv(f'{output_dir}/blockwise_g_vals_{id}_total.csv', index_label='block')
...
if __name__ == '__main__':
    ...
    for id in id_list:
        ...
        elec_intr = get_intr_sections(elec_intr)
        vdw_intr = get_intr_sections(vdw_intr)
        ...
        freq_dict = frequency_dict(part_deltaG_data_df)
